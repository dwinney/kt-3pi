...
%%
  \begin{equation}
    \phi = (2\sqrt{s} \; \sin \theta_s \; k(s) q(s))^2 =  \; \sin^2\theta_s \; \frac{\lambda_M(s) \; \lambda_\pi(s)}{4s} \; .
  \end{equation}
%%
...
%%
  \begin{equation}
    ...
    \mathcal{A}_\lambda(s,z_s) = \xi_{\lambda0}(z_s) \; K_{\lambda0}(s) \;  \sum_{j= |\lambda|}^\infty (2j+1) \, (k(s)q(s))^{j- |\lambda|} \; \hat{d}^j_{\lambda0}(\theta_s)  \; \hat{A}_{j\lambda}(s)
  \end{equation}
%%
...
%%
  \begin{align}
    ...
    \mathcal{A}_\lambda(s,t,u) = \epsilon_\mu^\lambda(p_A) \; \times \; &\bigg[
     ...
     +  \bar{F}^{(t)}(s,t,u)  \; (p_3 + p_2)^\mu
      +  \bar{F}^{(u)}(s,t,u)  \; (p_1 + p_3)^\mu \nonumber \\
    &+ \bar{G}^{(s)}(s,t,u) \; (p_1 - p_2)^\mu
    + \bar{G}^{(t)}(s,t,u)  \; (p_3 - p_2)^\mu
    + \bar{G}^{(u)}(s,t,u)  \; (p_1 - p_3)^\mu \bigg]
  \end{align}
%%
