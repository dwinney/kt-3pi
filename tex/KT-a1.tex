...
%%
  \begin{equation}
    ...
    a^{(s)}_{j\lambda}(s) = s^{\lambda/2} \, (p(s)q(s))^{j - |\lambda|} \, K^j_{\lambda 0} \; \hat{a}_{j\lambda}(s).
  \end{equation}
%%
Here
  \begin{equation}
    \label{eq:momenta}
    q(s) = \frac{\lambda^{1/2}(m_\pi^2, m_\pi^2, s)}{2\sqrt{s}} = \frac{\lambda_\pi^{1/2}}{2\sqrt{s}} \qquad \text{ and } \qquad k(s) = \frac{\lambda^{1/2}(M^2, m_\pi^2, s)}{2\sqrt{s}} = \frac{\lambda_M^{1/2}}{2\sqrt{s}} ,
  \end{equation}
...
The other kinematic factor, \(K^j_{\lambda0}\) arise  because near threshold \(\hat{a}_{j\lambda}(s)\) has additional power behavior of \(k(s)\) or \(q(s)\) corresponding to the dependence on \(j\) and \(L\) between helicity amplitudes and \(LS\) amplitudes (see discussions in \cite{Jackson1968,Franklin1966}). Near threshold:
%%
  \begin{gather}
    a_{j\lambda}(s) \sim k^{L_i}(s) \, q^{L_f}(s) \times (k(s)q(s))^{j- |\lambda|}
  \end{gather}
%%
...
%%
  \begin{equation}
    K_{\lambda 0}(s) = (2 \; \sqrt{s} \; k(s))^{|\lambda| + Y_i} \; (2 \; \sqrt{s} \; q(s))^{|\lambda| + Y_f} = \sqrt{\lambda_A}^{|\lambda| + Y_i} \; \sqrt{\lambda_\pi}^{|\lambda| + Y_f}\, .
  \end{equation}
%5
where
%%
...
%%
  \begin{equation}
    ...
    \mathcal{A}_\lambda(s,z_s) = s^{-\lambda/2} \, \xi_{\lambda0}(z_s) \;  \sum_{j= |\lambda|}^\infty (2j+1) \, (k(s)q(s))^{j-\lambda} \; \hat{d}^j_{\lambda0}(\theta_s) \; K^j_{\lambda0}(s) \; \hat{a}^{(s)}_{j\lambda}(s)
  \end{equation}
%%
with care to treat the \(\lambda =0, \;  j=0\) case as discussed above in \cref{eq:lilf}. We observe that we can rewrite \cref{eq:helicity-final} as
%%
  \begin{equation}
    \label{eq:kibble-helicity}
    \mathcal{A}_\lambda(s,z_s) =
     \sqrt{\phi^{|\lambda|}} \;
    \sum_{j =  |\lambda|}^\infty
    \sqrt{\lambda_A^{Y_i} \; \lambda_\pi^{Y_f}} \; (k(s)q(s))^{j - |\lambda|} \times (2j + 1) \; \hat{d}^j_{\lambda0}(\theta_s) \; \hat{a}^{(s)}_{j\lambda}(s)
  \end{equation}
%%
by introducing the Lorentz-covariant Kibble function \(\phi\)
%%
  \begin{equation}
    \phi = (2\sqrt{s} \; \sqrt{1-z_s^2} \; k(s) \, q(s))^2
    = s \, t \, u - m_\pi^2 \; (M^2 - m_\pi^2)^2 \; .
  \end{equation}
%%
%#########################################################################################################
%#########################################################################################################
...
Thus, \(\hat{a}_{j\lambda}(s) = 0\) unless \(j\) is even and so we can restrict the sum to even values.
 ...
 %%
   \begin{align}
     \label{eq:matching_F}
     F(s,t,u) = \; \hat{a}_{00}(s) + \, \frac{1}{\lambda_A} \sum_{j \text{ even}}^\infty &(2j+1) \, (k(s)\,q(s))^{j} \; \times \nonumber \\
...
\begin{align}
  \label{eq:iso-d-func}
   ...
   &+ \sum_{m} \sum_{j = 0}^{\jmax} (2j+1) \;
    ...
    %%
   + \sum_{m} \sum_{j = 0}^{\jmax} (-1)^{j + m} \; (2j+1) \;
...
%%
  \begin{align}
    ...
    \mathcal{A}_\lambda(s,t,u) = \epsilon_\mu^\lambda(p_A) \; \times \; &\bigg[
     F^{(s)}(s,t,u) \; (p_1 + p_2)^\mu
     + F^{(t)}(s,t,u) \; (p_3 + p_2)^\mu
      +F^{(u)}(s,t,u) \; (p_1 + p_3)^\mu \nonumber \\
    &+ G^{(s)}(s,t,u) \; (p_1 - p_2)^\mu
    + G^{(t)}(s,t,u) \; (p_3 - p_2)^\mu
    + G^{(u)}(s,t,u) \; (p_1 - p_3)^\mu \bigg]
  \end{align}
%%
where each \(F^{(x)}(s,t,u)\) and \(G^{(x)}(s,t,u)\) has a form like \cref{eq:matching_F,eq:matching_G} respectively with isobar partial waves in the specific channel.
%#########################################################################################################
%#########################################################################################################
%#########################################################################################################
...
We use the discussion in \cref{sec:kin-singularities} to factor out kinematic singularities since the constraint of unitarity is only on the kinematic-singularity-free isobar amplitudes defined by analogue of \cref{eq:kinematicfreepartialwave}.
%%
\begin{align}
  \label{eq:isobar}
  \mathcal{A}^{(I)}_\lambda&(s,t,u) =
